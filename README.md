...
- **引用数列**：注册第二个可排序列，来源可选 **Google Scholar（默认）/ OpenAlex / Semantic Scholar**，只显示所选来源的数字，不会互相顶替；悬停会显示数字来源。选 OpenAlex 时，位于本领域前 10% 的成果带 ▲ 标记；
- **被人机验证拦住也不会卡死**：Google Scholar 要验证时会弹提示、暂停读取，约 10 分钟后自动重试（继续被拦则 20 / 40 / 80 分钟翻倍，最长 2 小时）；那段时间该列显示「暂无」并说明原因，右键 →「打开 Google Scholar 验证页」可在浏览器里自己完成验证，回来点一次刷新即可；
- **批量匹配**：一次为多个条目查找 arXiv ID；
- **点赞趋势**：每次刷新记录当天的点赞数（保留最近若干天），列里显示每日变化，例如 `2979 ↑12`，增长达到阈值时标为“近期热门”；
- **11 种外观样式**：纯文本、玻璃胶囊、玻璃圆形、侧边强调块、莫兰迪低饱和、学术严谨、淡雅清新、活泼明快、细边框描边、双色拼接、数字角标，点赞列与引用列都会套用（名称不带括号说明，效果见下方预览页）；
- **颜色标记**：高赞绿色、低赞灰色、中间色可留空跟随主题；阈值与颜色都可自定义（纯文本、玻璃胶囊、玻璃圆形、细边框描边使用这些颜色，其余 7 种自带配色）；
...
**只会显示你选中的来源。** 这是刻意的：三个来源的统计口径不同，同一篇论文在 Google Scholar 和 OpenAlex 上差一倍很常见。所以当所选来源没有这条记录时，列里显示「暂无」，而不是拿另一个来源的数字充数。切换来源后，已有条目会在下一次读取时补齐新来源的数字。
...
2. **弹一条提示**告诉你发生了这件事、以及大约多久后重试；
3. **自动重试**：首次约 10 分钟后，若仍被拦则等待翻倍（20 → 40 → 80 分钟，最长 2 小时）；一旦成功读取，等待时间归零；
4. 那段时间里，引用列显示 **暂无**，把鼠标停上去会说明「Google Scholar 要求人机验证，约 X 分钟后自动重试」。

想马上解决：**右键任意条目 → 打开 Google Scholar 验证页**，在浏览器里完成验证，回到 Zotero 再点一次「刷新 alphaXiv 点赞」即可跳过等待、立刻重试。
...
- **引用数**：是否显示引用列、缓存多少天后重新读取、**引用数来源**（Google Scholar / OpenAlex / Semantic Scholar，默认 Google Scholar）；
