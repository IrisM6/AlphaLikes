...
alphaXiv 上没有这篇论文（或没有 DOI、没有 arXiv ID、标题太短到无从检索）时，格子留空，悬停写明**「alphaXiv 上没有找到这篇论文」**——这是结果，不是读取失败，也不会再自动重试。
...
- 页面上只要有 `Cited by`（或结果容器、「被引用次数」）字样就按读取成功处理，计数取的是**标题对得上的那一条结果自己**的数字；真正没有搜索痕迹的页面（Sorry 页、同意页、验证码）才按被拦处理。

### 读取节奏

Google Scholar 不喜欢机器一样的节奏，所以两次搜索之间**随机等 16–30 秒**，打开页面后先向下滚一点、**停留 4–8 秒**再读，每读完 **8–15 条**就歇 **15–40 分钟**。四个范围都能在 设置 → AlphaPulse 里改，每个框旁边写着建议值；下面一行**「当前：…」**把数字连成一句话，改一个字段就跟着变。正在读的时候，菜单最上面**每条文献一行**，写着这一条自己请求过几次、它自己什么时候重试；读到数字就清零。设置面板里那行同样只列文献，看不到整场会话的请求总数——那是队列的数字，不是任何一篇论文的。同一个等待在弹窗、格子悬停和设置里说的是同一个时间，列里没动静时看一眼就知道是在等还是停了。设置里有一个 **「清空等待时间（取消读取尝试）」** 按钮：清掉等待列表和上面的倒计时，之后不再自动重试，已经读到的数字不动；想再读就用右键 → **刷新引用数**。所有文献共用**同一个等待列表**：有文献在等重试时，新添加的条目排在后面（菜单里写着「排队等待读取（前面还有 N 条）」），不插队也不借用老条目的倒计时，只有确认没有任何文献正在读取时才发起下一次搜索；老条目的「马上自动重试」由列表自己的定时器兑现。
...
刷新只作用在选中的条目上，读取期间单元格显示 `…`，完成后弹一条结果提示。**两列互不等待**：刷新点赞数不会顺手去读引用数，一批引用数正在慢慢读的时候，新加进来的条目照样**立刻**读自己的点赞数。条目还没识别出 arXiv ID 时，「打开 alphaXiv 页面」会隐藏；选中多条时「手动填写引用量…」会隐藏（一个数字只属于一条）。
