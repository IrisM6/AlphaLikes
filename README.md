...
- **读取失败能说清楚失败在哪一步**：设置面板里的 **诊断读取** 会用当前设置真的各请求一次，把地址、请求头、状态码、异常、正文开头和代理设置汇总成报告并复制到剪贴板（同时写进调试日志）；列里的「N/A」在鼠标停靠时会说明是 403、限流、网络问题，还是页面里确实没有这个数字；
...
**列里的「N/A」会说明原因。** 把鼠标停在上面，可以看到是**被拒绝（403）**、**被限流（429）**、**服务器错误（5xx）**、**其他 4xx**、**网络 / DNS / 代理 / 超时**、**空响应**，还是**页面能打开但里面确实没有那个数字**——以前这几种情况都只显示同一句“没找到”，看不出要修哪里。
