...
现在这一版的做法：没有 arXiv ID 的条目会按 DOI 与标题查询多个免费学术 API，给候选打分后**只采用高置信度的结果**（中等相似度按“未找到”处理，不会猜、也没有需要你点确认的弹窗）；引用数有三个来源可选——**Google Scholar**（默认，用浏览器方式在 Zotero 自己的隐藏浏览器里真实打开公开搜索结果页，读页面里的 `Cited by`；这条路径不行时自动退回普通请求）、**OpenAlex**、**Semantic Scholar**，勾一个就**严格只显示那一个**，勾多个时显示其中**最大的数字**并注明来源。点赞与引用**各读各的、互不等待**（按网站分别排队，Google Scholar 的慢请求不会让点赞列停在加载中），**刷新只作用于选中的条目、读一条显示一条**，而 **Google Scholar 只在选中条目或手动刷新时读取**（OpenAlex / Semantic Scholar 仍然自动填，避免整列自动查询把自己送进验证），点赞数会记录**每日变化**（列里显示 `2979 ↑12`），两列都能按**固定阈值**或**当前列表的分位数**分档着色；外观有 **11 种样式**，每个颜色框都是取色面板，自带配色的样式会跟着你选的颜色走，点一下即可恢复默认；显示样式、高亮、范围筛选各自独立开关，设置改动即时生效。兼容 **Zotero 7 – 10**（每次发布前在四个真实版本上各跑一遍完整测试套件）。
...
- **被限流和被人机验证分开报**：429/503 说「这个地址被限流」，403 说「要求人机验证」，提示、鼠标说明和诊断报告都分别讲；
- **读取失败能说清楚失败在哪一步**：设置面板里的 **诊断读取** 会用当前设置真的各请求一次，把地址、请求头、状态码、异常、正文开头和代理设置汇总成报告并复制到剪贴板（同时写进调试日志）；报告里还有一项**指纹自检**：用两种读取方式各问一次 `tls.peet.ws`，打印它看到的 JA3 / JA4 / HTTP/2 指纹——这两个数字来自 Zotero 自己的 Gecko 引擎（和 Firefox 同一套 NSS 加密栈），用你平时的浏览器打开同一个地址就能对照，从而判断被拒的是「请求内容」还是「这个地址」；列里没有读到数字的格子**留空**（不再显示「N/A」），鼠标停靠时会说明是 403、限流、网络问题，还是页面里确实没有这个数字；
- **被人机验证拦住也不会卡死**：Google Scholar 要验证时**先静默等待**（头两次不打扰你），仍被拦才提示一次：说明原因、约多久后重试（10 分钟起，逐次翻倍，最长 2 小时）；连续 **4 轮**仍未通过就**停止自动重试**（继续重试只会让判定更差），并提示两个办法：右键 →「**重置谷歌会话**」清掉 Zotero 侧的 Google Cookie、重新建立会话再试，或等网络空闲时手动刷新。注意：在浏览器里打开搜索页用的是浏览器自己的登录状态，**不会解除插件这边的限制**；
...
- 断网、非 2xx 响应或页面结构变化时格子留空，不会中断 Zotero；
...
| **打开这篇论文的 Google Scholar 搜索页** | 在你自己的浏览器里用当前论文标题打开检索页（浏览器自己的会话，可核对数字）；不影响插件读取                                           |
| **打开这篇论文的 Google Scholar 搜索页** | 在**你自己的浏览器**里打开检索页（用浏览器自己的登录状态，可核对数字）。注意：它不会解除插件这边的限流——插件读的是 Zotero 自己的会话 |
| **重置谷歌会话**                         | 清除 Zotero 侧的 Google Cookie 并重试被拦住的条目（等同于换一个从没来过的浏览器）                                                    |
...
想马上重试：**右键任意条目 →「重置谷歌会话」**（清掉 Zotero 侧的 Google Cookie 并重建会话），或等 10 分钟让自动重试跑。浏览器里打开搜索页只是让你自己核对数字，用的是浏览器的会话，与插件无关。引用数按第一条例搜索结果读取，只有在标题足够相似时才会采用；对不上就显示「暂无」，不会拿别的来源的数字顶替。
...
- 请求带的是与本机浏览器一致的 User-Agent（而不是 `AlphaLikes/<版本号>` 这样的标识）：Google 会把陌生的非浏览器标识直接拒绝，这也是上面那种情况最常见的来源。
- 每个会话的**第一个** Google 请求是 Scholar 首页本身（浏览器也是先到站点再搜索），Google 设下的 cookie 由 Zotero 自己的 cookie 罐保留下来，之后的搜索都带着它们；每次会话只多这一次请求。
- **429/503 是「这个地址被限流」，403 是「要求人机验证」**：前者等待（或换网络）才有效，右键打开验证页对它帮助不大；后者可以用验证页自己完成一次。诊断报告里会写明本次会话打开首页的结果，正好用来区分这两种。
...
**还是读不到就按「诊断读取」。** 设置 → AlphaLikes → 「刷新与网络」里有一个 **诊断读取** 按钮：它用你当前的设置真的各请求一次 alphaXiv 与 Google Scholar，把请求地址、请求头（含 User-Agent）、HTTP 状态、异常、返回正文的开头、代理设置和当前等待状态写成一份报告并**复制到剪贴板**，面板上会显示“已复制 / 没成功”；报告同时也写进 Zotero 的调试日志（**帮助 → 调试输出日志**，找 `[AlphaLikes]` 开头的行）。把整段贴出来就能直接定位是哪一步出的问题——它只读不写，不会改动缓存、设置或 `Extra`。

**列里空白的格子会说明原因。** 把鼠标停在上面，可以看到是**被拒绝（403）**、**被限流（429）**、**服务器错误（5xx）**、**其他 4xx**、**网络 / DNS / 代理 / 超时**、**空响应**，还是**页面能打开但里面确实没有那个数字**——以前这几种情况都只显示同一句“没找到”，看不出要修哪里。
...
test/                            Zotero 集成测试与纯逻辑测试（229 个用例，在 Zotero 7/8/9/10 上各跑一遍）
test/pane.test.ts                打开真实的设置窗口，点取色面板、切着色依据、按恢复默认
test/clear.test.ts               清除只删自己的行；清除后不再被自动写回，刷新后可以恢复
test/scholar-request.test.ts     发往 Google 的请求头、同意 cookie，403 按验证处理，以及默认请求通道没有丢掉 this
test/diagnose.test.ts            诊断报告的内容与不写缓存、不改设置，以及失败原因码
