...
- 数字实在读不到时可以自己填：右键 → **手动填写引用量…**。
- 想继续工作，可以同时勾上 OpenAlex 或 Semantic Scholar。**在浏览器里打开检索页不会解除插件这边的限制**——两边是各自的会话，那个入口只是给你自己核对数字用的。

### 读取节奏

Google Scholar 不喜欢机器一样的节奏，所以两次搜索之间会**随机等 16–30 秒**，打开页面后先向下滚一点、**停留 4–8 秒**再读，每读完 **8–15 条**就歇 **15–40 分钟**。四个范围都在 设置 → AlphaPulse 里可以自己改，每个框旁边写着建议值；下面还有一行**「当前：…」**，把这些数字连成一句话，改一个字段它就跟着变。

正在读的时候，插件菜单顶部会写着**本次会话已请求多少次、距离下次请求还有多久**——列里一时没有动静时，看一眼就知道它是在按节奏等，还是停下了。
...
两个刷新动作**各读各的**，互不影响，也只作用在选中的条目上；读取期间单元格显示 `…`，完成后弹一条结果提示。工具菜单里那份永远在（没选中条目也能打开 Google Scholar 检索页），右键里那份跟着选中状态出现；条目还没识别出 arXiv ID 时，「打开 alphaXiv 页面」自己会隐藏，选中多条时「手动填写引用量…」会隐藏（一个数字只属于一条）。
...
- **Google Scholar 被拦住**：插件会自己清掉这边的 Google Cookie 再重试，你不用管；同时勾上 OpenAlex / Semantic Scholar 可以先照常使用。
- **就是想手动填一个引用数**：右键 → **手动填写引用量…**，填进去的数字会一直显示，自动读取不再覆盖它；把输入框清空就交回自动读取。
