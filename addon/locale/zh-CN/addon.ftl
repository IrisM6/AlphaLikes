...
pref-citations-scholar-note = Google Scholar 没有公开 API，插件读取的是搜索结果页上的「Cited by」数字。Google 可能会要求人机验证或限流：这时插件不会用别的来源顶替这个数字，而是暂停读取、弹一条提示，并在约 10 分钟后自动重试（每次都失败就翻倍，最长 2 小时）。右键菜单里的「打开 Google Scholar 验证页」可以在浏览器里自己完成验证，然后点一次刷新即可。读取过于频繁更容易被拦，因此请不要把刷新间隔调得太小。
...
pref-citations-source-note = 只显示所选来源的数字，不会用其他来源顶替：三个来源的口径并不一致（例如 Google Scholar 收录预印本、学位论文与书籍，和 OpenAlex 的数字可能差一倍）。默认为收录最广的 Google Scholar；它被人机验证拦住时会暂停重试（有提示），此时该列显示「暂无」而不是别的来源的数字。列里悬停会显示数字来源。
